...
$('.shop-button-header .amount').bind('changedprice', function(){
	...
	setTimeout(
		function(){
			$this.html($('.widget_shopping_cart_content .total .amount').html());}, 10000)
});
	...
	if (window_width < 752) {
		responsive_menu = true;
		    ...
		    $('.header_nav .nav li').mouseenter(function(){ 
               ...
               var liHeight = 0;
               $(this).children('ul').children('li').each(function(){  liHeight += $(this).outerHeight()+1; } ); 
	...
	if($windowWidth > 975){
		...
		if(animateNavBar == true){
			if(!$('header').hasClass('sticky')){
				$('header .nav > li > a').stop(true).animate({'padding-top': navPadding, 'padding-bottom': navPadding}, 200,function(){
			...
			if($('header').hasClass('sticky')){
				setTimeout(function(){
					...
					searchPadding = 23;
					headerMinHeight = 132;
					$('header .nav > li > a, header .nav > ul > li > a').stop(true).animate({'padding-top': navPadding, 'padding-bottom': navPadding}, 200,function(){
						var navLinkHeight = $('.navbar .nav > li > a').outerHeight();
